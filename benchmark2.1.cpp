...
const wchar_t* NPOINT_DOC_ID = L"XXXXXXXXXXXXXXXX";
...
int currentFontSize=21,currentLangIndex=0,g_winW=1400,g_winH=700;
        ...
        for(DWORD g=0;g<gCount;g++){
            ...
            for(int t=0;t<cnt;t++){
                futuros.push_back(std::async(std::launch::async,[func,duracao,g,t]() -> long long {
                    GROUP_AFFINITY ga={};ga.Group=(WORD)g;ga.Mask=(KAFFINITY)1<<(t%64);
...
void DesenharPainelModal(HDC hdc,RECT&area,bool isMulti,int idxA,int idxB,bool meuPcVsOutro){
    ...
    if(meuPcVsOutro){
        ...
        sA=isMulti?meuUltimoTotalMulti:meuUltimoTotalSingle;
        sB=isMulti?pcsCadastrados[idxB].totalMultiCore:pcsCadastrados[idxB].totalSingleCore;
    } else {
        nA=pcsCadastrados[idxA].nome;
        nB=pcsCadastrados[idxB].nome;
        scA=isMulti?pcsCadastrados[idxA].scoresMultiCore:pcsCadastrados[idxA].scoresSingleCore;
        scB=isMulti?pcsCadastrados[idxB].scoresMultiCore:pcsCadastrados[idxB].scoresSingleCore;
        sA=isMulti?pcsCadastrados[idxA].totalMultiCore:pcsCadastrados[idxA].totalSingleCore;
        sB=isMulti?pcsCadastrados[idxB].totalMultiCore:pcsCadastrados[idxB].totalSingleCore;
    }
    ...
    if(sA>=sB){sV=sA;sP=sB;nV=nA;nP=nB;scV=scA;scP=scB;}
    else{sV=sB;sP=sA;nV=nB;nP=nA;scV=scB;scP=scA;}
    // % SEMPRE POSITIVA
    double diff=0;
    if(sP>0) diff=((sV-sP)/sP)*100.0;
    // Contagem de testes ganhos
    int gV=0,gP=0;
    for(int i=0;i<100;++i){
        if(scV[i]>scP[i])gV++;
        else if(scP[i]>scV[i])gP++;
    }
    ...
    SetTextColor(hdc,cT);
    wchar_t pts[128];
    swprintf_s(pts,L"%s: %.1f",t.modalPts,sV);
...
LRESULT CALLBACK ModalProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam){
    ...
    case WM_COMMAND:
        if(LOWORD(wParam)==ID_MODAL_BTN_CLOSE) FecharModal();
        break;
    case WM_CLOSE:
        FecharModal();
        return 0;
...
void FecharModal(){
    if(hModalWnd){DestroyWindow(hModalWnd);hModalWnd=NULL;}
    DestruirFontesModal();
}

// =============================================================================
// RANKING MODELO 3 - LADO A LADO COM SCROLL
// =============================================================================
...
void DesenharColunaRanking(HDC hdc,int colLeft,int colRight,int topY,int bottomY,
    ...
    SelectObject(hdc,op);
    DeleteObject(hp);
    // Calcular linhas visiveis
    int listTop=topY+headerH+5;
    int maxLines=(bottomY-listTop)/lineH;
    if(maxLines<1)maxLines=1;
    // Score maximo para barras
    double maxScore=0;
    for(size_t idx:indices){
        double sc=isMulti?pcsCadastrados[idx].totalMultiCore:pcsCadastrados[idx].totalSingleCore;
    ...
    for(int i=startIdx;i<endIdx;i++){
        ...
        SelectObject(hdc,opL);DeleteObject(hpL);
        int xC=colLeft+8;
        // Rank
        if(rank==1){
            ...
            wchar_t rs[16];swprintf_s(rs,L"%d",rank);
            RECT rr={xC,ly+2,xC+30,ly+lineH-2};
        ...
        SetTextColor(hdc,corH);
        wchar_t scStr[64];swprintf_s(scStr,L"%.1f PTS",score);
        int scW=120;
    ...
    if((int)indices.size()>maxLines){
        ...
        FillRect(hdc,&rt2,hbTr);DeleteObject(hbTr);
        double tR=(double)maxLines/(double)indices.size();
        int tH=(int)(sbH*tR);if(tH<20)tH=20;
        ...
        FillRect(hdc,&rth,hbTh);DeleteObject(hbTh);
    }
    SelectObject(hdc,of);
}
...
void DesenharRanking(HDC hdc,RECT&rc){
    ...
    // Preparar indices ordenados
    std::vector<size_t>idxM,idxS;
    for(size_t i=1;i<pcsCadastrados.size();++i){idxM.push_back(i);idxS.push_back(i);}
    ...
    std::sort(idxS.begin(),idxS.end(),[](size_t a,size_t b){
        return pcsCadastrados[a].totalSingleCore>pcsCadastrados[b].totalSingleCore;});
    // Desenhar colunas
...
LRESULT CALLBACK RankingProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam){
    switch(uMsg){
    case WM_ERASEBKGND:{
        HDC h=(HDC)wParam;RECT rc;GetClientRect(hwnd,&rc);
        HBRUSH hb=CreateSolidBrush(RGB(10,15,30));FillRect(h,&rc,hb);DeleteObject(hb);
        return 1;
    }
    case WM_PAINT:{
        PAINTSTRUCT ps;HDC hdc=BeginPaint(hwnd,&ps);
        HDC hm=CreateCompatibleDC(hdc);RECT rc;GetClientRect(hwnd,&rc);
        HBITMAP hbm=CreateCompatibleBitmap(hdc,rc.right,rc.bottom);
        HBITMAP obm=(HBITMAP)SelectObject(hm,hbm);
        DesenharRanking(hm,rc);
        BitBlt(hdc,0,0,rc.right,rc.bottom,hm,0,0,SRCCOPY);
        SelectObject(hm,obm);DeleteObject(hbm);DeleteDC(hm);
    ...
    case WM_MOUSEWHEEL:{
        ...
        int midX=rc.right/2;
        int maxS=(int)pcsCadastrados.size()-1-10;if(maxS<0)maxS=0;
        if(pt.x<midX){
    ...
    case WM_COMMAND:
        if(LOWORD(wParam)==ID_RANKING_BTN_CLOSE)FecharRanking();
        break;
    case WM_CLOSE:
        FecharRanking();return 0;
    }
    return DefWindowProcW(hwnd,uMsg,wParam,lParam);
}
...
void AbrirRanking(){
    ...
    g_rankScrollMulti=0;g_rankScrollSingle=0;
    CriarFontesRanking();
    int rw=1400,rh=750;
    int sw=GetSystemMetrics(SM_CXSCREEN),sh=GetSystemMetrics(SM_CYSCREEN);
...
void FecharRanking(){
    if(hRankingWnd){DestroyWindow(hRankingWnd);hRankingWnd=NULL;}
...
void VerificarBotoesResumo(){
    ...
    if(hComboCpuA){
        ...
        if(p>=0){int idx=(int)SendMessageW(hComboCpuA,CB_GETITEMDATA,p,0);if(idx>0)cA=true;}
    }
    if(hComboCpuB){
        int p=(int)SendMessageW(hComboCpuB,CB_GETCURSEL,0,0);
        if(p>=0){int idx=(int)SendMessageW(hComboCpuB,CB_GETITEMDATA,p,0);if(idx>0)cB=true;}
    }
    if(cA&&cB)ShowWindow(hBtnResumo,SW_SHOW);
    else ShowWindow(hBtnResumo,SW_HIDE);
    if(benchmarkRealizado){
        ...
        if(p>=0){
            ...
            if(idx>0)ShowWindow(hBtnVerResultado,SW_SHOW);
            else ShowWindow(hBtnVerResultado,SW_HIDE);
        }
    } else {
...
void PosicionarControlesTopo(){
    ...
    int wCL=(int)(150*esc),wCF=(int)(90*esc),wCC=(int)(280*esc);
    int wCA=(int)(450*esc),wCB=(int)(450*esc);
    auto cp=[&](HWND l,HWND c,const wchar_t*t,int w){
        int wl=MedirTextoLargura(hFontAtual,t)+pad;
        int tn=wl+eL+w+eC;
        if(x+tn>lim&&x>15){x=15;y+=aL;}
    ...
    cp(hStaticFont,hComboFont,traducoes[currentLangIndex].labelFont,wCF);
    ...
    // Pular para proxima linha - botoes sempre visiveis
    x=15; y+=aL;
    ...
    y+=aC+(int)(15*esc);
    int aLog=g_winH-y-15;if(aLog<200)aLog=200;
...
void AtualizarFontesSimples(int fs){
    ...
    hFontAtual=CreateFontW(fs,0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,
        DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,
        DEFAULT_QUALITY,DEFAULT_PITCH|FF_DONTCARE,L"Arial");
    HWND ctr[]={hStaticLang,hComboLang,hStaticFont,hComboFont,
        hStaticCompare,hComboCompare,hStaticCpuA,hComboCpuA,
        hStaticCpuB,hComboCpuB,hBtnRun,hStaticStatus,
        hBtnResumo,hBtnVerResultado,hBtnRanking};
    for(HWND h:ctr)if(h)SendMessageW(h,WM_SETFONT,(WPARAM)hFontAtual,TRUE);
    if(hEditLog){
...
void AtualizarIdiomaInterface(int li){
    ...
    SetWindowTextW(hStaticFont,traducoes[li].labelFont);
...
void AtualizarListaComparacao(){
    ...
    for(size_t i=1;i<pcsCadastrados.size();++i)idx.push_back(i);
    ...
    pc(hComboCompare);pc(hComboCpuA);pc(hComboCpuB);
}

void AtualizarPainelComparativo(){
    if(!benchmarkRealizado){
        SetWindowTextW(hEditLog,traducoes[currentLangIndex].avisoRodarPrimeiro);return;
    }
    int pos=(int)SendMessageW(hComboCompare,CB_GETCURSEL,0,0);
    int ci=-1;
    if(pos>=0)ci=(int)SendMessageW(hComboCompare,CB_GETITEMDATA,pos,0);
    bool tc=(ci>0&&ci<(int)pcsCadastrados.size());
    std::wstring lt=L"";
    lt+=L"===============================================================\r\n";
    lt+=traducoes[currentLangIndex].headerResult;
    lt+=L"\r\n===============================================================\r\n\r\n";
    lt+=traducoes[currentLangIndex].compMultiCore;lt+=L"\r\n";
    {wchar_t c[512];
    if(tc)swprintf_s(c,L"%-35s | %12s | %12s | %10s\r\n",
    ...
    else swprintf_s(c,L"%-35s | %12s\r\n",
        traducoes[currentLangIndex].colTeste,traducoes[currentLangIndex].colSeuPC);
    lt+=c;g_cabecalhoAtual=c;g_modoCpuXCpuAtivo=false;}
    lt+=L"-----------------------------------------------------------------------\r\n";
    for(int i=0;i<100;++i){wchar_t l[512];
    if(tc){double d=0;
        if(pcsCadastrados[ci].scoresMultiCore[i]>0)
            d=((meuUltimoScoreTestsMulti[i]-pcsCadastrados[ci].scoresMultiCore[i])/
            pcsCadastrados[ci].scoresMultiCore[i])*100.0;
        swprintf_s(l,L"%-35s | %12.1f | %12.1f | %9.1f%%\r\n",
            nomesTestesPT[i],meuUltimoScoreTestsMulti[i],
            pcsCadastrados[ci].scoresMultiCore[i],d);
    }else swprintf_s(l,L"%-35s | %12.1f\r\n",nomesTestesPT[i],meuUltimoScoreTestsMulti[i]);
    lt+=l;}
    wchar_t t2[256];double dtm=0;
    if(tc){if(pcsCadastrados[ci].totalMultiCore>0)
        dtm=((meuUltimoTotalMulti-pcsCadastrados[ci].totalMultiCore)/
        pcsCadastrados[ci].totalMultiCore)*100.0;
    swprintf_s(t2,L"\r\n%s: %.1f PTS | %.1f PTS | %.1f%%\r\n",
        traducoes[currentLangIndex].compTotal,meuUltimoTotalMulti,
        pcsCadastrados[ci].totalMultiCore,dtm);
    }else swprintf_s(t2,L"\r\n%s: %.1f PTS\r\n",
        traducoes[currentLangIndex].compTotal,meuUltimoTotalMulti);
    lt+=t2;lt+=L"\r\n";
    lt+=traducoes[currentLangIndex].compSingleCore;lt+=L"\r\n";
    {wchar_t c[512];
    if(tc)swprintf_s(c,L"%-35s | %12s | %12s | %10s\r\n",
        traducoes[currentLangIndex].colTeste,traducoes[currentLangIndex].colSeuPC,
        traducoes[currentLangIndex].colOutroPC,traducoes[currentLangIndex].colDif);
    ...
    for(int i=0;i<100;++i){wchar_t l[512];
    if(tc){double d=0;
        if(pcsCadastrados[ci].scoresSingleCore[i]>0)
            d=((meuUltimoScoreTestsSingle[i]-pcsCadastrados[ci].scoresSingleCore[i])/
            pcsCadastrados[ci].scoresSingleCore[i])*100.0;
        swprintf_s(l,L"%-35s | %12.1f | %12.1f | %9.1f%%\r\n",
            nomesTestesPT[i],meuUltimoScoreTestsSingle[i],
            pcsCadastrados[ci].scoresSingleCore[i],d);
    }else swprintf_s(l,L"%-35s | %12.1f\r\n",nomesTestesPT[i],meuUltimoScoreTestsSingle[i]);
    lt+=l;}
    double dts=0;
    if(tc){if(pcsCadastrados[ci].totalSingleCore>0)
        dts=((meuUltimoTotalSingle-pcsCadastrados[ci].totalSingleCore)/
        pcsCadastrados[ci].totalSingleCore)*100.0;
    swprintf_s(t2,L"\r\n%s: %.1f PTS | %.1f PTS | %.1f%%\r\n",
        traducoes[currentLangIndex].compTotal,meuUltimoTotalSingle,
        pcsCadastrados[ci].totalSingleCore,dts);
    }else swprintf_s(t2,L"\r\n%s: %.1f PTS\r\n",
        traducoes[currentLangIndex].compTotal,meuUltimoTotalSingle);
    lt+=t2;lt+=L"\r\n=================================================================\r\n";
    if(tc){
        if(dtm>0){lt+=L"\r\n";lt+=traducoes[currentLangIndex].msgMaisRapido;
            wchar_t b[100];swprintf_s(b,L"%.1f%%) ***\r\n",dtm);lt+=b;
        }else if(dtm<0){lt+=L"\r\n";lt+=traducoes[currentLangIndex].msgMaisLento;
            wchar_t b[100];swprintf_s(b,L"%.1f%%) ***\r\n",dtm);lt+=b;
        }else{lt+=L"\r\n";lt+=traducoes[currentLangIndex].msgEmpate;lt+=L"\r\n";}
    }else{lt+=L"\r\n";lt+=traducoes[currentLangIndex].msgSelecioneParaComparar;lt+=L"\r\n";}
    lt+=L"=================================================================\r\n";
    SetWindowTextW(hEditLog,lt.c_str());
}

void AtualizarPainelComparativoCpuXCpu(int idxA,int idxB){
    if(idxA<=0||idxB<=0||idxA>=(int)pcsCadastrados.size()||
       idxB>=(int)pcsCadastrados.size()||idxA==idxB)return;
    ...
    for(int s=0;s<2;++s){
        ...
        swprintf_s(c,L"%-35s | %55.55s | %55.55s | %10s\r\n",
            traducoes[currentLangIndex].colTeste,a.nome.c_str(),b.nome.c_str(),
            traducoes[currentLangIndex].colDif);
        lt+=c;
        if(s==0){g_cabecalhoAtual=c;g_modoCpuXCpuAtivo=true;}}
        lt+=L"---------------------------------------------------------------------------------------------------------------------------\r\n";
        const double*sA=m?a.scoresMultiCore:a.scoresSingleCore;
        const double*sB=m?b.scoresMultiCore:b.scoresSingleCore;
        for(int i=0;i<100;++i){wchar_t l[512];double d=0;
            if(sA[i]>0&&sB[i]>0){double ma=(sA[i]>sB[i])?sA[i]:sB[i];double me=(sA[i]<sB[i])?sA[i]:sB[i];d=((ma-me)/me)*100.0;}else d=0;
            swprintf_s(l,L"%-35s | %55.1f | %55.1f | %9.1f%%\r\n",
                nomesTestesPT[i],sA[i],sB[i],d);lt+=l;}
        double tA=m?a.totalMultiCore:a.totalSingleCore;
        double tB=m?b.totalMultiCore:b.totalSingleCore;
        double dT=0;if(tA>0&&tB>0){double ma2=(tA>tB)?tA:tB;double me2=(tA<tB)?tA:tB;dT=((ma2-me2)/me2)*100.0;}
        wchar_t ts[512];
        swprintf_s(ts,L"\r\n%-12s %55.1f PTS | %55.1f PTS | %9.1f%%\r\n",
            traducoes[currentLangIndex].compTotal,tA,tB,dT);
        lt+=ts;lt+=L"\r\n";
    }
    lt+=L"=================================================================\r\n";
    SetWindowTextW(hEditLog,lt.c_str());
...
void ObterSelecaoCpuXCpu(int&idxA,int&idxB){
    idxA=-1;idxB=-1;
    if(hComboCpuA){int p=(int)SendMessageW(hComboCpuA,CB_GETCURSEL,0,0);
        if(p>=0)idxA=(int)SendMessageW(hComboCpuA,CB_GETITEMDATA,p,0);}
    if(hComboCpuB){int p=(int)SendMessageW(hComboCpuB,CB_GETCURSEL,0,0);
        if(p>=0)idxB=(int)SendMessageW(hComboCpuB,CB_GETITEMDATA,p,0);}
}

void AtualizarPainelPrincipal(){
    int iA=-1,iB=-1;
    ObterSelecaoCpuXCpu(iA,iB);
    bool mc=(iA>0&&iB>0&&iA<(int)pcsCadastrados.size()&&
             iB<(int)pcsCadastrados.size()&&iA!=iB);
    if(mc)AtualizarPainelComparativoCpuXCpu(iA,iB);
    else if(benchmarkRealizado)AtualizarPainelComparativo();
    VerificarBotoesResumo();
}
...
void ExecutarBenchmarkThread(HWND hwnd,int langIndex){
    ...
    try{
        EnableWindow(hBtnRun,FALSE);EnableWindow(hComboLang,FALSE);
        EnableWindow(hComboCompare,FALSE);EnableWindow(hComboFont,FALSE);
...
// Fallback: ler do registro se hardware_concurrency falhar
if(nT==0||nT<2){
    HKEY hKey;DWORD val=0,sz=sizeof(val);
    if(RegOpenKeyExW(HKEY_LOCAL_MACHINE,L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",0,KEY_READ,&hKey)==ERROR_SUCCESS){
        RegQueryValueExW(hKey,L"~MHz",NULL,NULL,(LPBYTE)&val,&sz);
        RegCloseKey(hKey);
    }
    // Contar quantos processadores existem no registro
    unsigned int count=0;
    for(int i=0;i<512;i++){
        wchar_t path[128];swprintf_s(path,L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\%d",i);
        HKEY hK2;if(RegOpenKeyExW(HKEY_LOCAL_MACHINE,path,0,KEY_READ,&hK2)==ERROR_SUCCESS){count++;RegCloseKey(hK2);}else break;
    }
    if(count>nT)nT=count;
}
if(nT==0)nT=4;if(nT>4000)nT=4000;
        ...
        double fe[100]={
            ...
            100000.0,100000.0,100000.0,100000.0,100000.0,100000.0,100000.0,100000.0,100000.0,100000.0};
        int dpT=1;
        meuUltimoTotalMulti=0;meuUltimoTotalSingle=0;meuUltimoNumThreads=nT;
        ...
        {wchar_t c[256];swprintf_s(c,L"%-35s | %12s\r\n",
            traducoes[langIndex].colTeste,traducoes[langIndex].colPontuacao);lt+=c;}
        lt+=L"-----------------------------------------------------------------------\r\n";
        SetWindowTextW(hEditLog,lt.c_str());
        for(int i=0;i<100;++i){
            double sm=rodarMultiThread(tf[i],nT,dpT,fe[i]);
            meuUltimoScoreTestsMulti[i]=sm;meuUltimoTotalMulti+=sm;
            wchar_t l[256];swprintf_s(l,L"%-35s | %12.1f\r\n",nomesTestesPT[i],sm);lt+=l;
            SetWindowTextW(hEditLog,lt.c_str());
            SendMessageW(hEditLog,EM_SETSEL,(WPARAM)-1,(LPARAM)-1);
            SendMessageW(hEditLog,EM_SCROLLCARET,0,0);
            wchar_t st[256];swprintf_s(st,L"%s %d/100",traducoes[langIndex].compMultiCore,i+1);
            SetWindowTextW(hStaticStatus,st);
        }
        SetWindowTextW(hStaticStatus,traducoes[langIndex].statusRodandoSingle);
        lt+=L"\r\n";lt+=traducoes[langIndex].compSingleCore;lt+=L"\r\n\r\n";
        {wchar_t c[256];swprintf_s(c,L"%-35s | %12s\r\n",
            traducoes[langIndex].colTeste,traducoes[langIndex].colPontuacao);lt+=c;}
        lt+=L"-----------------------------------------------------------------------\r\n";
        SetWindowTextW(hEditLog,lt.c_str());
        ...
        for(int i=0;i<100;++i){
            ...
            meuUltimoScoreTestsSingle[i]=ss;meuUltimoTotalSingle+=ss;
            wchar_t l[256];swprintf_s(l,L"%-35s | %12.1f\r\n",nomesTestesPT[i],ss);lt+=l;
            SetWindowTextW(hEditLog,lt.c_str());
            SendMessageW(hEditLog,EM_SETSEL,(WPARAM)-1,(LPARAM)-1);
            SendMessageW(hEditLog,EM_SCROLLCARET,0,0);
            wchar_t st[256];swprintf_s(st,L"%s %d/100",traducoes[langIndex].compSingleCore,i+1);
            SetWindowTextW(hStaticStatus,st);
        }
        ...
        if(!en){
            PCReferencia np;np.nome=nrpc;np.numThreads=nT;
            np.totalMultiCore=meuUltimoTotalMulti;np.totalSingleCore=meuUltimoTotalSingle;
            for(int i=0;i<100;++i){
                np.scoresMultiCore[i]=meuUltimoScoreTestsMulti[i];
                np.scoresSingleCore[i]=meuUltimoScoreTestsSingle[i];
            }
            pcsCadastrados.push_back(np);nR=true;
        }
        if(nR)SalvarListaNuvemNativo();
        AtualizarListaComparacao();
        ShowWindow(hStaticCompare,SW_SHOW);ShowWindow(hComboCompare,SW_SHOW);
        SetWindowTextW(hStaticStatus,traducoes[langIndex].statusPronto);
        if(IsWindow(hwnd)){
            EnableWindow(hBtnRun,TRUE);EnableWindow(hComboLang,TRUE);
            EnableWindow(hComboCompare,TRUE);EnableWindow(hComboFont,TRUE);
...
LRESULT CALLBACK WindowProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam){
    ...
    case WM_CREATE:{
        ...
        int ww=(1400<sw)?1400:sw,wh=(700<sh)?700:sh;
        ...
        CHARFORMAT2W cf={0};cf.cbSize=sizeof(cf);
        cf.dwMask=CFM_SIZE|CFM_FACE|CFM_COLOR|CFM_BOLD;
        cf.yHeight=21*10;wcscpy_s(cf.szFaceName,L"Consolas");
        cf.crTextColor=RGB(0,0,0);cf.dwEffects=CFE_BOLD;
        SendMessageW(hEditLog,EM_SETCHARFORMAT,SCF_ALL,(LPARAM)&cf);
        AtualizarFontesSimples(21);
        const wchar_t*langs[]={L"Portugues (Brasil)",L"English (US)",L"Espanol",L"Frances",
            L"Alemao",L"Italiano",L"Chines",L"Russo",L"Coreano",L"Holandes",L"Polones",L"Japones"};
        for(int i=0;i<12;++i)SendMessageW(hComboLang,CB_ADDSTRING,0,(LPARAM)langs[i]);
        SendMessageW(hComboLang,CB_SETCURSEL,0,0);
        const wchar_t*fsz[]={L"Pequena (18)",L"Media (21)"};
        for(int i=0;i<2;++i)SendMessageW(hComboFont,CB_ADDSTRING,0,(LPARAM)fsz[i]);
    ...
    case WM_COMMAND:
        if(LOWORD(wParam)==ID_COMBO_LANG&&HIWORD(wParam)==CBN_SELCHANGE){
            ...
            if(li>=0&&li<12)AtualizarIdiomaInterface(li);
        }else if(LOWORD(wParam)==ID_COMBO_FONT&&HIWORD(wParam)==CBN_SELCHANGE){
            int fi=(int)SendMessageW(hComboFont,CB_GETCURSEL,0,0);
            int sz[]={18,21,27,33};
            if(fi>=0&&fi<4)AtualizarFontesSimples(sz[fi]);
            ...
            int iA=-1,iB=-1;ObterSelecaoCpuXCpu(iA,iB);
            if(iA>0&&iB>0&&iA!=iB)AbrirModalResumo(iA,iB,false);
        }else if(LOWORD(wParam)==ID_BTN_VER_RESULTADO&&HIWORD(wParam)==BN_CLICKED){
            if(benchmarkRealizado){
                int p=(int)SendMessageW(hComboCompare,CB_GETCURSEL,0,0);
                if(p>=0){int idx=(int)SendMessageW(hComboCompare,CB_GETITEMDATA,p,0);
                if(idx>0)AbrirModalResumo(0,idx,true);}
            }
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR pCmdLine,int nCmdShow){
    ...
    wc.hCursor=LoadCursor(NULL,IDC_ARROW);
    RegisterClassW(&wc);
    HWND hwnd=CreateWindowExW(0,CN,L"Benchmark Multi-Core C++",
