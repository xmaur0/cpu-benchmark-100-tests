...
const wchar_t* NPOINT_DOC_ID = L"your id npoit";
...
std::wstring GetLocalCachePath(){return GetLocalDataDir()+L"\\cache.dat";}

bool SalvarJsonLocalCriptografado(const std::string& json){
    if(json.empty()||json=="{\"data\":[]}")return false;
    DATA_BLOB dataIn{};dataIn.pbData=(BYTE*)json.data();dataIn.cbData=(DWORD)json.size();
    DATA_BLOB entropy{};entropy.pbData=(BYTE*)g_entropiaLocal;entropy.cbData=sizeof(g_entropiaLocal);
    DATA_BLOB dataOut{};
...
int ContarRegistrosJson(const std::string& jn){
    ...
    size_t pa=jn.find("\"data\":[");
    if(pa==std::string::npos)return 0;
    int count=0;size_t p=pa;
...
bool SalvarListaNuvemNativo(){
    ...
    bool nuvemOk=!jan.empty()&&jan.find("\"data\"")!=std::string::npos;
...
void SincronizarDadosInicializacao(HWND hwnd){
    ...
    bool nuvemOk=!jsonNuvem.empty()&&jsonNuvem.find("\"data\"")!=std::string::npos;

    int nLocal=ContarRegistrosJson(jsonLocal);
    int nNuvem=nuvemOk?ContarRegistrosJson(jsonNuvem):-1;

    // Uniao completa em memoria (isso e o que garante zero perda de dados).
    if(!jsonLocal.empty())MesclarDadosNaMemoria(jsonLocal);
    if(nuvemOk)MesclarDadosNaMemoria(jsonNuvem);
