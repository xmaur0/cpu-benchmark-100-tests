...
const wchar_t* NPOINT_URL = L"https://api.npoint.io/xx";
...
IdiomaTextos traducoes[] = {
    ...
    { L"Benchmark Multi-Core C++ (100 Tests)", L"Idioma:", L"Comparar com:", L"Fonte:", L"Ordenar por:", L"Modo:",
      ...
      L">> TOTAL <<", L"Executando 100 testes (Multi + Single)... Aguarde...", L"*** SEU PC E MAIS RAPIDO (+", L"*** SEU PC E MAIS LENTO (", L"*** EMPATE TECNICO ***", L"*** SELECIONE UM PC PARA VER A COMPARACAO ***", L"Erro", L"Ocorreu um erro durante o benchmark.", L"Ocorreu um erro desconhecido.", L"Erro no benchmark.", L"Aguarde", L"Aguarde o termino do benchmark antes de fechar.", L"CPU A:", L"CPU B:", {} },
    // 1: English
    ...
    { L"Benchmark Multi-Core C++ (100 Tests)", L"Idioma:", L"Comparar con:", L"Fuente:", L"Ordenar por:", L"Modo:",
      ...
      L">> TOTAL <<", L"Ejecutando 100 pruebas (Multi + Single)... Espera...", L"*** TU PC ES MAS RAPIDA (+", L"*** TU PC ES MAS LENTA (", L"*** EMPATE TECNICO ***", L"*** SELECCIONA UN PC PARA VER LA COMPARACION ***", L"Error", L"Ocurrio un error durante el benchmark.", L"Ocurrio un error desconocido.", L"Error en el benchmark.", L"Espera", L"Espera a que termine el benchmark antes de cerrar.", L"CPU A:", L"CPU B:", {} },
    // 3: French
    ...
    { L"C++ Benchmark (100 Tests)", L"Yazyk:", L"Sravnit:", L"Shrift:", L"Sort:", L"Rezhim:",
      ...
      L">> ITOGO <<", L"Vypolnenie 100 testov (Multi + Single)... Pozhaluysta podozhdite...", L"*** VASH PK BYSTREE (+", L"*** VASH PK MEDLENNEE (", L"*** TEKHNICHESKAYA NICHYA ***", L"*** VYBERITE PK DLYA SRAVNENIYA ***", L"Oshibka", L"Proizoshla oshibka vo vremya testa.", L"Proizoshla neizvestnaya oshibka.", L"Oshibka testa.", L"Pozhaluysta podozhdite", L"Pozhaluysta dozhdites okonchaniya testa pered zakrytiem.", L"CPU A:", L"CPU B:", {} },
    // 8: Korean
...
// FUNÇÃO CORRIGIDA: Declarada antes do uso
...
void PosicionarControlesTopo() {
    ...
    // Larguras aumentadas em 50% para leitura dos nomes de CPU
    int wComboLang = (int)(150 * escala);
    int wComboFont = (int)(90 * escala);
    int wComboCompare = (int)(420 * escala); 
    int wComboCpuA = (int)(330 * escala);    
    int wComboCpuB = (int)(330 * escala);    
    ...
    // Log ocupa todo o espaço restante (IA removida)
...
void AtualizarPainelComparativoCpuXCpu(int idxA, int idxB) {
    ...
    std::wstring nomeAcurto = a.nome.substr(0, 12);
    std::wstring nomeBcurto = b.nome.substr(0, 12);
    std::wstring logText = L"";
    logText += L"===============================================================\r\n";
    logText += traducoes[currentLangIndex].headerResult;
    logText += L"\r\n===============================================================\r\n\r\n";
    for (int secao = 0; secao < 2; ++secao) {
        ...
        {
            wchar_t cabecalho[512];
            swprintf_s(cabecalho, L"%-35s | %12.12s | %12.12s | %10s\r\n",
                       traducoes[currentLangIndex].colTeste, nomeAcurto.c_str(),
                       nomeBcurto.c_str(), traducoes[currentLangIndex].colDif);
            logText += cabecalho;
        }
        logText += L"-----------------------------------------------------------------------\r\n";
...
void ExecutarBenchmarkThread(HWND hwnd, int langIndex) {
    ...
    try {
        ...
        // Limite aumentado para 4000 núcleos conforme solicitado
        ...
        // Agora a função ObterNomeCompletoCPU está corretamente declarada acima
