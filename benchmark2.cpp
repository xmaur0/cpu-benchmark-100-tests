...
// Traduções completas e corrigidas para todos os 12 idiomas
    ...
    if (hFile) {
        ...
        if (posArray != std::string::npos) {
            ...
            while ((pos = resposta.find("{\"nome\"", pos)) != std::string::npos) {
                ...
                if (posTotalMulti != std::string::npos) try { totalMulti = std::stod(resposta.substr(posTotalMulti + 13)); } catch(...) {}
                size_t posTotalSingle = resposta.find("\"totalSingle\":", endNome);
                if (posTotalSingle != std::string::npos) try { totalSingle = std::stod(resposta.substr(posTotalSingle + 14)); } catch(...) {}
                size_t posThreads = resposta.find("\"threads\":", endNome);
                if (posThreads != std::string::npos) try { threads = std::stoi(resposta.substr(posThreads + 10)); } catch(...) {}
                ...
                if (posEndFirstArray != std::string::npos) scoresSingleVec = ExtrairArrayDouble(resposta, posEndFirstArray);
                ...
                for (size_t k = 1; k < pcsCadastrados.size(); ++k) {
                    if (pcsCadastrados[k].nome == wNome) {
                        ...
                        if (totalMulti > pcsCadastrados[k].totalMultiCore && !scoresMultiVec.empty()) {
                            ...
                            for (int s = 0; s < 100 && s < (int)scoresMultiVec.size(); ++s) pcsCadastrados[k].scoresMultiCore[s] = scoresMultiVec[s];
                            atualizou = true;
                        }
                        if (totalSingle > pcsCadastrados[k].totalSingleCore && !scoresSingleVec.empty()) {
                            pcsCadastrados[k].totalSingleCore = totalSingle;
                            for (int s = 0; s < 100 && s < (int)scoresSingleVec.size(); ++s) pcsCadastrados[k].scoresSingleCore[s] = scoresSingleVec[s];
                            atualizou = true;
                        }
...
// =============================================================================
// ESPAÇAMENTO AUMENTADO PARA 55 CARACTERES + ALINHAMENTO CORRIGIDO NO TOTAL
// =============================================================================
void AtualizarPainelComparativoCpuXCpu(int idxA, int idxB) {
    ...
    // Truncamento aumentado para 55 caracteres
    ...
    for (int secao = 0; secao < 2; ++secao) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            // Valores alinhados à largura de 55
        ...
        // Usando %55.1f para garantir que os números fiquem na mesma coluna dos testes acima
        wchar_t total[512];
